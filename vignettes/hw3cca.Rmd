...
Here, we generate data randomly. The result of `cca_simple` and `cancor` is the same.  There are two canonical correlation coefficients. `xcoef` is coefficient for x variables so this is linear combination coefficients. `ycoef` is coefficient for y variables so this is linear combination coefficients.
...
test_that("cca_simple function", {
  ...
  expect_equal(cca_simple(test_data[, 1:4], test_data[, 5:8])[1],
               cancor(test_data[, 1:4], test_data[, 5:8])[1], tolerance = 0.0001)
  expect_equal(cancor(pop, oec)[1], cca_simple(pop, oec)[1], tolerance = 0.0001)
  expect_equal(cancor(pop, oec)[1], cca_simple_cpp(pop, oec)[1], tolerance = 0.0001)
})
```
...
You can run `cca_simple_cpp` in your computer. It will faster after compiling.
